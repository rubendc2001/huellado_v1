...
function consultarEntradas() {
    fetch('/consultar_entradas')
        .then(response => response.json())
        .then(data => {
            console.log(data);
            pintarTablaEntradas(data);
        })
        .catch(error => {
...
function pintarTablaEntradas(data) {
    const tableBody = document.getElementById('usuariosEntradaTable').getElementsByTagName('tbody')[0];
    tableBody.innerHTML = '';
